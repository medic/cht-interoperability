...
module.exports = [
  ...
  {
    name: 'interoperability.follow_up.title',
    icon: 'icon-follow-up',
    title: 'task.interoperability.follow_up.title',
    appliesTo: 'reports',
    appliesToType: ['INTEROP_FOLLOW_UP'],
    resolvedIf: function (contact, report, event, dueDate) {
      const startTime = Math.max(addDays(dueDate, -event.start).getTime(), report.reported_date + 1);
      //reported_date + 1 so that source ds_follow_up does not resolve itself
      const endTime = addDays(dueDate, event.end + 1).getTime();
      return isFormArraySubmittedInWindow(contact.reports, ['interop_follow_up'], startTime, endTime);
    },
    actions: [
      {
        type: 'report',
